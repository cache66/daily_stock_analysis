...
class FastReviewStockOverviewItem(BaseModel):
    code: str = Field(..., description="Stock code")
    name: Optional[str] = Field(None, description="Stock name")
    stock_review_lane: Optional[str] = Field(None, description="Stock-centered review lane")
    stock_review_lane_label: Optional[str] = Field(None, description="Display label for stock-centered review lane")
    tier: Optional[str] = Field(None, description="Fast-review tier such as core/watch")
    ab_bucket: Optional[str] = Field(None, description="Fast-review A/B bucket")
    review_stage_label: Optional[str] = Field(None, description="Review stage label")
    driver_label: Optional[str] = Field(None, description="Driver label")
    priority_score: Optional[float] = Field(None, description="Fast-review priority score")
    strategy_count: int = Field(0, description="Number of triggered strategy keys")
    signal_keys: List[str] = Field(default_factory=list, description="Source signal keys")
    signal_types: List[str] = Field(default_factory=list, description="Source signal types")
    triggered_strategies: List[str] = Field(default_factory=list, description="Triggered strategy keys for this stock")
    chart_evidence_summary: Optional[str] = Field(None, description="Chart evidence summary")
    earnings_evidence_summary: Optional[str] = Field(None, description="Earnings evidence summary")
    stock_context_summary: Optional[str] = Field(None, description="Stock context summary")
    today_change_pct: Optional[float] = Field(None, description="Today's percentage change")
    pe_ratio: Optional[float] = Field(None, description="PE ratio")
    report_period_label: Optional[str] = Field(None, description="Latest report period label")
    revenue_yoy: Optional[float] = Field(None, description="Revenue YoY percentage")
    net_profit_yoy: Optional[float] = Field(None, description="Net profit YoY percentage")
    roe: Optional[float] = Field(None, description="ROE percentage")
    earnings_strategy_score: Optional[float] = Field(None, description="Hybrid earnings-strategy score")
    earnings_strategy_gate_status: Optional[str] = Field(None, description="Hybrid earnings-strategy gate status")
    earnings_quality_score: Optional[float] = Field(None, description="Earnings quality score")
    earnings_quality_cycle_phase: Optional[str] = Field(None, description="Earnings quality cycle phase")
    capital_profile_score: Optional[float] = Field(None, description="Capital profile score")
    relative_strength_score: Optional[float] = Field(None, description="Relative strength score")
    breakout_quality_score: Optional[float] = Field(None, description="Breakout quality score")
    market_expectation_institution_count: Optional[int] = Field(None, description="Institution expectation count")
    net_profit_amount: Optional[float] = Field(None, description="Latest reported net profit amount")
    primary_board_name: Optional[str] = Field(None, description="Primary board name")
    preferred_industry_label: Optional[str] = Field(None, description="Preferred business-led industry label")
    theme_label: Optional[str] = Field(None, description="Resolved theme label")
    mainline_judgement: Optional[str] = Field(None, description="Mainline judgement")
    reason_summary: Optional[str] = Field(None, description="Readable reason summary")
    display_reason_summary: Optional[str] = Field(None, description="Compact reason summary for daily review UI")
    cause_tags_zh: Optional[str] = Field(None, description="Chinese cause tags")
    latest_trade_date: Optional[str] = Field(None, description="Latest trade date in source artifact")
    pure_chart_quality_passed: Optional[bool] = Field(None, description="Whether pure-chart read gate passed")


class FastReviewStockOverviewResponse(BaseModel):
    snapshot_date: str = Field(..., description="Snapshot date")
    total: int = Field(..., description="Total stock overview rows")
    source_run_dir: str = Field(..., description="Resolved run directory")
    source_csv_path: str = Field(..., description="Resolved stock overview CSV path")
    lane_summary: Dict[str, int] = Field(default_factory=dict, description="Stock review lane summary")
    signal_summary: Dict[str, int] = Field(default_factory=dict, description="Triggered signal summary")
    items: List[FastReviewStockOverviewItem] = Field(default_factory=list, description="Stock overview rows")


class PersonalStrategyDefinition(BaseModel):
    ...
    group: str = Field(..., description="Strategy group")
    group_label: str = Field(..., description="Strategy group display label")
    mode: str = Field(..., description="Strategy mode")
    aliases: List[str] = Field(default_factory=list, description="Signal aliases matched to this strategy")
    role: str = Field(..., description="Current role")
    logic: str = Field(..., description="Main judgement logic")


class PersonalStrategyMatch(BaseModel):
    id: str = Field(..., description="Strategy id")
    name: str = Field(..., description="Strategy display name")
    short_name: str = Field(..., description="Short display name")
    group: str = Field(..., description="Strategy group")
...
class PersonalStrategyMatrixItem(FastReviewStockOverviewItem):
    matched_strategies: List[PersonalStrategyMatch] = Field(default_factory=list, description="Matched personal strategies")
    matched_strategy_ids: List[str] = Field(default_factory=list, description="Matched personal strategy ids")
    matched_strategy_count: int = Field(0, description="Matched personal strategy count")
    quality_score: float = Field(0, description="Unified display-layer quality score")
    quality_band: str = Field(..., description="Unified quality band: recommended/watch/weak")
    quality_label: str = Field(..., description="Display label for the quality band")
    quality_summary: str = Field(..., description="Compact quality summary")
    quality_flags: List[str] = Field(default_factory=list, description="Quality warnings and downgrade reasons")
    view_lane: str = Field(..., description="Display lane: short_term/long_term/watch")
    view_lane_label: str = Field(..., description="Display label for the strategy view lane")
    view_lane_summary: str = Field(..., description="Compact explanation for the strategy view lane")


class PersonalStrategyMatrixResponse(BaseModel):
    snapshot_date: str = Field(..., description="Snapshot date")
    total: int = Field(..., description="Total filtered stock rows")
    source_run_dir: str = Field(..., description="Resolved run directory")
    source_csv_path: str = Field(..., description="Resolved stock overview CSV path")
    lane_summary: Dict[str, int] = Field(default_factory=dict, description="Filtered stock review lane summary")
    signal_summary: Dict[str, int] = Field(default_factory=dict, description="Filtered triggered signal summary")
    strategy_summary: Dict[str, int] = Field(default_factory=dict, description="Strategy hit counts before strategy-id filtering")
    strategies: List[PersonalStrategyDefinition] = Field(default_factory=list, description="Personal strategy catalog")
    items: List[PersonalStrategyMatrixItem] = Field(default_factory=list, description="Stock-centered strategy matrix rows")
