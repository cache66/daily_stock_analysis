...
vi.mock('./pages/PersonalStrategiesPage', () => ({
  default: () => <div data-testid="personal-strategies-page">Personal strategies</div>,
}));
...
describe('App routing behavior', () => {
  ...
  it('routes /personal-strategies to the personal strategy matrix after auth is ready', async () => {
    window.history.pushState({}, '', '/personal-strategies');

    render(<App />);

    expect(await screen.findByTestId('personal-strategies-page')).toBeInTheDocument();
    expect(setCurrentRoute).toHaveBeenCalledWith('/personal-strategies');
    expect(screen.queryByTestId('home-page')).not.toBeInTheDocument();
  });
