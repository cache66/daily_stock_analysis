...
2026-09-25 addendum (股息线阶段收口):

- 个人策略新增「吃股息 / 股息线（防守区）」支线，本阶段已完成 MVP → 因子 v2/v3 → 平台对照 → 点时回测 → 行业结构分析的完整闭环，并**暂停迭代**（以后再来讨论）。
- 全过程总结与后续方向：`docs/个人策略文档/吃股息策略-实施与验证总结.md`（恢复工作先读它的 §9）。
- 运行入口：`scripts/select_dividend_income_candidates.py`（筛选）、`scripts/backtest_dividend_income.py`（回测）；产物在 `data/dividend_income/<日期>/`。
- 回测核心结论：合格池等权年化 +9.7%（含息）优于中证红利价格口径；评分排序/趋势/低波因子未验证出增量。

2026-07-11 addendum (optional signal convergence after probe):

- 这轮收口不再继续盲调阈值，而是先处理扩展信号的读层边界：
  - `trend_leader` 的 `fallback` 候选不再允许进入首页体系；
  - `continuous_up` 不再作为独立候选信号，只保留“节奏观察”语义；
  - `monthly_slow_rise` 先解决运行链路，优先本地缓存和白名单 universe，再谈质量。
- `trend_leader`：
  - 首页聚合现在至少要求不是 `selection_mode=fallback`；
  - 且不能是 `trend_score<=0`；
  - 且不能带 `weak_trend_structure`。
  - 原始 `trend_leader_unified_candidates.csv` 仍保留 fallback 诊断行，便于后续看为什么没出严格候选，但这些行不再抬高首页关系、优先级或趋势标签。
- `continuous_up`：
  - 信号文案明确改成“仅作节奏观察，需其他主信号共振”；
  - 若当天没有和 `earnings / hundred_day_high / trend_leader / monthly_slow_rise / daily_slow_rise / long_base_release` 共振，就直接清空 continuous 候选，不再单独捞出 `深圳华强` 这类噪声节奏票。
- `monthly_slow_rise`：
  - 新增 `--cache-only`，会先按本地 `history cache` 过滤 universe，没有缓存的样本直接跳过，不补抓；
  - 新增 `--universe-codes-file`，允许先用白名单收窄 universe；
  - `run_fast_review_bundle.py` 转发 monthly 时默认走 `cache-only`，优先解决白天运行稳定性。
- 2026-07-10 受限扩展矩阵验证：
  - 命令：`python scripts/run_personal_strategy_matrix.py --snapshot-date 2026-07-10 --extended-signals --bundle-limit 300 --skip-prewarm --safe-mode --skip-persist-snapshots --output-dir data/manual_runs/personal_strategy_matrix_extended_converged_v1_2026-07-10 --log-level WARNING`
  - 结果：
    - `trend_leader` 原始候选 `17` 条，但首页 `趋势补充` 为 `0`，说明 fallback 已被首页读层完全隔离；
    - `continuous_up_ratio / continuous_up_streak` 都是 `no_rows`，没有再单独浮出噪声票；
    - `monthly_slow_rise` 在 `cache-only + limit=300` 下直接 `no_rows` 返回，未出现长时间挂起；
    - 整轮 `safe-mode` 耗时约 `119s`，成功结束。

2026-07-11 addendum (chart-quality convergence after manual review):

- 本轮人工复盘结论：
  - 现在策略经常选到“历史上曾经好看，但最近已经走坏”的票；
  - 现在策略对“图形是否健康”的判断不够严格，容易把震荡做 T、上影线多、回撤大的票当成强票；
  - 现在策略对“业绩/催化的持续性”判断不够，只要某一季数据好，或者曾经逻辑顺，就容易入选，但目标是“接下来还会继续演绎”的票。
- 后续个人策略收敛方向：
  - 先看大方向：行业、涨价、景气、AI、机器人、复苏等逻辑要明确，不能只靠单季偶然好；
  - 再看业绩持续性：不是一季爆发就算，要能推演后续 1-2 个季度；
  - 最后看图形健康度：底部起来后小步上涨、一浪高过一浪、回撤可控，不是随大盘大起大落。
- 图形票审美口径：
  - 阳线占比要高，表示持续有人买；
  - 上影线不能太多，太多往往代表做 T、抛压重、筹码不稳；
  - 回撤不能太深，尤其不能上涨后大幅吐回；
  - 日线不能乱，月线至少要向上，最好日线、周线、月线方向一致；
  - 不能只因历史图形好就入选，最近明显转弱必须降级或剔除。
- 本轮样本反馈：
  - `603991 领先股份`：不是绝对差，但业绩、估值、图形都只算一般；
  - `002025 航天电器`：历史走势反复，最近日线/月线都不够强，是典型误选；
  - `600428 中远海特`：整体走势和质量感不够，不符合稳定上涨和确定性；
  - `605289 罗曼股份`：近期表现差，不能因过去某段数据好继续高权重；
  - `601899 紫金矿业`：月线更像走 A，不符合中长期趋势健康度；
  - `300054 鼎龙股份`：过去图形可能漂亮，但最近已经弱了；
  - `002803 吉宏股份`：随大盘大起大落，回撤和波动偏大，不符合稳步上行。
- 已落地的第一步收敛：
  - `daily_slow_rise` 和 `long_base_release` 增加近 `20` 日上影线压力过滤；
  - 两个图形策略导出 `upper_shadow_day_ratio_20d / upper_shadow_avg_pct_20d / upper_shadow_max_pct_20d`；
  - 两个图形策略增加近端月线趋势确认字段 `monthly_uptrend_*`，用于压掉日线片段好看但月线未保持向上的样本；
  - 保持原有历史窗口，不因为月线粗读拉长白天全市场扫描。

2026-07-11 addendum (long-term position / valuation overlay):

- 新增长期位置/估值叠加复盘：
  - `scripts/build_long_term_review_overlay.py`
  - `src/services/long_term_position_service.py`
- 默认只读已有 `fast_review_stock_overview.csv` 和本地 `data/cache/history`，不扫全市场。
- 读取指标：
  - 1/2 年价格分位；
  - 120/250 日均线；
  - 月线 6/12 月均线；
...
- 周期股估值暂不只看 PE：
  - 资源、航运、锂电材料等缺 PB 时标记为“周期股需补PB/周期位置”；
  - 避免把紫金矿业、中远海特这类票只按当季业绩或短线形态直接前置。
- 分位口径：
  - `估算PB2年分位`：用当前 PB 按历史收盘价比例回推，未考虑每期净资产变化，只用于快速粗读；
  - `历史PB分位`：来自 Tushare `daily_basic` 的真实历史 PB 序列，优先级高于估算 PB 分位；
  - `样本内PB分位`：只在本次复盘 22 只样本内比较，不能替代全行业估值分位；
  - `同催化PB分位`：只在同一明确催化组样本数不少于 3 时显示。
- 当前限制：
  - 本机 Tushare `daily_basic` 账号侧限频很低，本次实测会触发 `1次/分钟` 或 `1次/小时` 级限制；
  - 脚本已做 provider-side cooldown，遇到限频后本轮停止继续请求，只使用已缓存数据和估算分位；
  - 因此真实 `历史PB分位` 需要逐步积累缓存，不能白天一次性全量拉完。
- 2026-07-10 样本验证：
  - 输出目录：`data/manual_runs/review_explicit_catalysts_2026-07-10/2026-07-10/review/`
  - 全量 22 条逐票补腾讯 quote 后，PB 标签分布：`PB偏高 15`、`PB中性 5`、`PB偏低 2`；
  - Tushare `daily_basic` 本轮只成功缓存 `002025 航天电器`，其余因账号限频回退估算 PB 分位；
  - `600428 中远海特`：长期上行，PB `1.44`、估算 PB 2 年分位 `89.68%`、样本内 PB 分位 `4.55%`，结合航运催化继续看；
  - `601899 紫金矿业`：中位震荡/待确认，PB `3.89`、估算 PB 2 年分位 `66.07%`，资源催化存在但当前未站上 MA120/MA250；
  - `603986 兆易创新`：高位强趋势，PB `17.35`、估算 PB 2 年分位 `98.21%`、样本内 PB 分位 `100%`，控制追高。

2026-07-11 addendum (optional signal probe conclusion):

- `trend_leader / continuous_up / monthly_slow_rise` 暂不纳入默认每日复盘。
- 2026-07-10 探针结论：
  - `continuous_up` 命中的 `000062 深圳华强` 质量不够，偏噪声；
  - `trend_leader` 正选 `002008 大族激光` 只能作为边缘观察，不足以进入默认主线；
  - `monthly_slow_rise` 白天全市场跑会触发明显限速等待，需先加强预筛，不适合直接全量默认跑。
- 默认每日复盘继续只跑：
  - `earnings`
  - `hundred_day_high`
  - `daily_slow_rise`
...
2026-07-11 addendum (cycle catalyst read-layer):

- 业绩票新增一个轻量读层：`cycle_catalyst_*`。
- 当前先覆盖“存储涨价/复苏 + 业绩兑现”：
  - `603986 兆易创新`
  - `001309 德明利`
- 这层不新增外部扫描，不扩大默认策略，只复用已有 `earnings` 行、本地业务别名、业绩周期字段和营收/净利增速。
- 2026-07-10 离线重建验证：
  - `603986 兆易创新` 保持 `watch`；
  - `strategy_focus` 和 `stock_overview` 新增 `cycle_catalyst_label=存储涨价/复苏 + 业绩兑现`；
  - 输出目录：`data/manual_runs/review_cycle_catalyst_2026-07-10/2026-07-10/review/`。

2026-07-11 addendum (explicit catalyst registry):

- 明确催化先做第一批，不追求覆盖所有小众行业：
  - 存储涨价/复苏
  - AI 算力链
  - 机器人产业链
  - 资源涨价/景气
  ...
  - 航运景气/运价
- 新增 registry：
  - `src/services/industry_catalyst_registry.py`
- 这层会给业绩票补 `cycle_catalyst_*`，并回填业务标签；命中明确催化后，可以作为纯业绩票的确认依据之一。
- 2026-07-10 放松复盘离线重建：
  - 输出目录：`data/manual_runs/review_explicit_catalysts_2026-07-10/2026-07-10/review/`
  - 首页从 `16` 只扩到 `22` 只；
  - `天赐材料` 识别为 `锂电材料复苏/涨价 + 业绩兑现`；
  - `工业富联` 识别为 `AI算力链景气 + 业绩兑现`；
  - `紫金矿业 / 江西铜业 / 藏格矿业` 等识别为 `资源涨价/景气 + 业绩兑现`；
  - `中远海特` 识别为 `航运景气/运价 + 业绩兑现`。

2026-07-11 addendum (accumulation watch):

- 纯图形票不再一刀切降级，也不再叫确定性主线。
- 新增 `accumulation_watch / 吸筹观察`：
  - 图形走顺、阳线/慢涨/长平台释放代表有人买，可以观察；
  - 但没有业绩、行业催化、涨价、资金、板块确认前，不进入“确定性主线”。
- `chart_first / 图形优先` 现在只给“图形 + 非图形确认”的样本。
- 2026-07-10 离线重建验证：
  - `603986 兆易创新` 是 `earnings_first / 确定性主线`；
  - `603991 领先股份`、`002025 航天电器`、`300684 中石科技`、`002821 凯莱英` 是 `accumulation_watch / 吸筹观察`；
  - 输出目录：`data/manual_runs/review_accumulation_watch_2026-07-10/2026-07-10/review/`。

2026-07-11 addendum (long-base release read-layer confirmation):

- 问题不是扩展信号不够，而是默认图形策略读层把部分有效 `long_base_release` 票压低了。
- 纯 `long_base_release` 现在不再只依赖过严的 `pure_chart_quality_passed`，而是拆成两个阅读状态：
  - `confirmed`：已经重新走顺，适合作为图形优先候选；
  - `recovery_watch`：底部释放后近端仍横住、回撤可控，但还没达到确认突破。
- `confirmed` 的核心条件：
  - 交易日新鲜；
  - 站上 `MA10`；
  - 近 `10` 日仍有涨幅确认；
  - 近 `10` 日回撤和连阴可控；
  - release 阶段回撤可控。
- `recovery_watch` 的核心条件：
  - release 阶段已经有明显离底涨幅；
  - 近 `5/10` 日没有明显破位；
  - 近端回撤和 release 阶段最大回撤都仍可控。
- 同时补齐 bundle CSV 读层里的 `recent_return_10d / recent_drawdown_10d / above_ma10` 等字段，避免子策略已经导出质量指标但读层没拿到。
- 2026-07-10 离线重建验证：
  - `002025 航天电器`、`300684 中石科技`、`603991 领先股份` 标记为 `confirmed`；
  - `002821 凯莱英`、`603519 立霸股份` 标记为 `recovery_watch`；
  - 已明显破位的长平台样本仍保持低优先级。

2026-07-11 addendum (interface relation map + stock detail drawer):

- 新增界面/API/产物关系图：
  - [`个人策略界面接口关系图.md`](./个人策略界面接口关系图.md)
- Web `/personal-strategies` 现在支持点击股票打开详情抽屉：
  - 展示单票快照、策略命中、图形证据、业绩证据、行业/题材、原始信号键；
  - 详情仍然复用 `GET /api/v1/signals/personal-strategy-matrix` 返回的数据；
  - 页面不在详情里重新拉行情、财报或重新计算策略阈值。

2026-07-10 addendum (personal strategy matrix runner):

- 新增统一运行入口：
  - `python scripts/run_personal_strategy_matrix.py --snapshot-date <YYYY-MM-DD>`
- 这条入口把个人策略矩阵生成拆成两步顺序执行：
  - 先跑 `scripts/warm_local_strategy_cache.py`，预热常用 A 股样本的 K 线历史和业绩上下文缓存；
  - 再跑 `scripts/run_fast_review_bundle.py`，生成 `fast_review_stock_overview.csv`，供 Web 个人策略矩阵读取。
- 默认产物路径：
  - 缓存预热：`data/runtime/cache_prewarm/<snapshot-date>/`
  - 个人策略矩阵：`data/manual_runs/personal_strategy_matrix/<snapshot-date>/review/fast_review_stock_overview.csv`
  - 运行摘要：`data/manual_runs/personal_strategy_matrix/<snapshot-date>/personal_strategy_matrix_run_summary.json`
- 试跑命令：
  - `python scripts/run_personal_strategy_matrix.py --snapshot-date <YYYY-MM-DD> --dry-run`
  - `python scripts/run_personal_strategy_matrix.py --snapshot-date <YYYY-MM-DD> --bundle-limit 200 --prewarm-limit 200`
- 扩展信号命令：
  - `python scripts/run_personal_strategy_matrix.py --snapshot-date <YYYY-MM-DD> --extended-signals`
- 当前边界：
  - 默认仍按 `config/local_strategy_profile.json` 跑每日核心信号；
  - `--extended-signals` 只覆盖 `run_fast_review_bundle.py` 当前支持的扩展信号；
  - `board_cycle_scan / theme_core_mapper / commodity_price_pass_through / shortline_hub` 这些专题工具还没有统一并入这个 runner，若已有独立 snapshot 或复盘产物，矩阵读层可以识别它们写入的 signal key，但本入口不会凭空计算专题信号。

2026-07-10 addendum (personal strategy matrix UI):

- Web 侧新增个人策略矩阵第一版入口：`/personal-strategies`
- 页面定位：
  - 顶部先列策略过滤器
  - 每个策略显示当前命中的股票数
  - 点击策略后，下方股票列表只保留命中股票
  - 股票行只展示命中的策略，未命中策略不占空间
  - 策略说明按分组折叠
- 页面仍然只读 `fast_review_stock_overview.csv` 的后端矩阵 API 输出，不重新计算策略阈值。
- 当前页面策略名称、作用与主要判断逻辑记录在：
  - [`个人策略界面策略清单.md`](./个人策略界面策略清单.md)
- 后端策略 registry 真源在：
  - `src/services/personal_strategy_matrix_service.py`
- 页面聚合 API：
  - `GET /api/v1/signals/personal-strategy-matrix?snapshot_date=<YYYY-MM-DD>`

2026-07-10 addendum (stock-centered query interface):

- 这轮继续沿着“股票优先”方向补读层接口，不再新增策略阈值。
- 当前 `fast_review_stock_overview.csv` 不只是报告产物，也有稳定 API 入口：
  - `GET /api/v1/signals/fast-review-stock-overview?snapshot_date=<YYYY-MM-DD>`
  - 可选 `code=<stock-code>`，例如只看 `300475` 触发了哪些策略
- 返回重点字段：
...
- 这个接口刻意只读已有 CSV，不主动做行情/财报网络补齐：
  - 用途是复盘查询和策略归因
  - 不应该再被 `Akshare / Tushare` 尾部不稳定拖慢
- 后续如果继续把更多个人策略合并到“单票证据视图”，优先保证它们能写入 `signal_keys` 和证据摘要，再由这个 stock overview 读层统一承接。

2026-07-10 addendum (earnings observation manual profile + exports):

- 仓库里原本就有 `scripts/collect_earnings_observation_snapshots.py`，但它之前更像 schedule 尾部快照任务：
  - 默认按 `earnings_surprise balanced` 入池
  - 只写 snapshot，不直接导出手工复盘文件
- 这轮把它补成了可直接用于“单独业绩观察池”的工具：
  - 新增 `--entry-strategy-profile`
    - 默认仍是 `balanced`
    - 手工运行可显式切到 `relaxed`
  - 新增 `--output-dir`
    - 会导出：
...
- 实际意义：
  - 主策略 `earnings_surprise balanced` 可以继续负责“首页/主候选”
  - 较宽但仍值得持续观察的业绩票，改由 `earnings_observation` 观察池单独承接
  - 后续如果要看“这批业绩票有没有继续走强”，优先看 `earnings_observation_active`

2026-07-10 addendum (stock-centered fast review alignment):

- 这轮不是再加一条新策略，而是把默认每日复盘再往“单票视角”推一步。
- 当前新增一个明确的中间产物：
  - `fast_review_stock_overview.csv`
  - `fast_review_stock_overview.md`
- 这两个文件的定位不是替代 `fast_review_summary.md`，而是补上一个稳定回答：
  - 这只股票今天到底命中了哪些策略
  - 它更偏 `图形优先 / 业绩优先 / 双确认 / 趋势补充`
  - 当前图形证据、业绩证据和一句话理由分别是什么
- 同时，`strategy_focus` 的底层聚合也不再只偏向 `earnings / hundred_day_high / trend_leader`：
  - `daily_slow_rise`
  - `long_base_release`
  - 这两条现在也会先按股票并进复盘聚合层
  - 不需要等它们“恰好和别的策略重叠”才有资格进入单票视角
- 实际意义：
  - 默认每日阅读从“按策略翻文件”继续收口成“先看股票，再看证据”
  - 后续如果要继续做 `业绩 × 板块 × 位置 × 想象空间`，优先应该加在这个单票聚合层，而不是继续把策略拆得更散

2026-07-11 addendum (daily review quality gate + daily_slow_rise review profile):

- 这轮处理的是“首页结果质量”，不是继续扩大策略数量。
- `daily_slow_rise` 新增 `review_balanced` profile，并成为默认每日复盘口径：
  - 比 `accelerating` 更适合日常复盘观察
  - 适度放宽回撤和单日涨幅限制
  - 同时增加近 `10` 日回撤护栏，避免近期明显破位的票因为历史区间够强而进入首页
- `strategy_focus` 对纯 `earnings-only` 票新增阅读层确认：
  - `fast_review_focus_gate_passed=true` 只作为最低门槛
  - 如果 `market_expectation_reference_label=unknown`，还需要价格、相对强度、资金画像或机构预期摘要里至少一项确认
  - 不满足的票会降到 `low_priority`，并标记 `review_gate_status=earnings_only_unconfirmed`
- `strategy_focus` 对纯 `earnings-only` 票继续新增缓存价格健康门槛：
  - 默认只读本地 `data/cache/history`，不新增联网扫描
  - 读取近 `10/20` 日收益、近 `20` 日回撤、`MA20/MA60`、近端月线方向
  - 如果业绩票已经出现明显回落、大回撤、跌破中期均线或月线转弱，且缺少明确行业催化，会降到 `low_priority`
  - 有明确行业催化的弱走势业绩票保留为 `review_gate_status=earnings_price_pullback_watch`，按“回撤观察”继续出现在复盘中
  - 缺少明确催化的弱走势样本标记 `review_gate_status=earnings_price_health_weak`，并导出 `earnings_price_health_*` 字段用于复盘原因定位
- 实际意义：
  - `earnings_focus` 仍保留全量业绩候选
  - 每日首页优先留给“财报过线、市场愿意看、当前图形还没走坏”的票，以及更顺的图形票

2026-07-09 addendum (Akshare no-data semantics after Tushare empty history):

- 这轮不调 `daily_slow_rise` 阈值，处理的是剩余运行语义噪音。
- 当前明确收口为：
  - `Tushare` 空历史后仍然继续回退 `Akshare`
  - 如果这时 `Akshare EM` 只是处于既有 backoff，而腾讯等剩余渠道也只是返回空历史，这类后续 miss 会被归类成 no-data
  - 不再因为 `EM backoff + 其余渠道 empty` 这一组合，把后续一串样本继续累计成 `AkshareFetcher` 的 manager 级熔断
- 实际意义：
  - `daily_slow_rise` 后续若仍然偏少，更接近“真实候选不足”或免费源覆盖波动，而不是被误熔断噪音放大
  - 回看日志时，可以把注意力更多放在真实 `EM` 首次硬失败和真实候选分布，而不是一长串 backoff 连带失败

2026-07-09 addendum (shared Tushare rate limiter + no-data fallback restored):

- 这轮不改任何策略阈值，处理的是“个人策略实跑时的运行护栏”。
- 现状明确为：
  - 全市场快扫里的 `Tushare` 走进程级共享限流，不再允许多个 `TushareFetcher` 实例各自单独按 45/min 记账后叠加撞上账号侧 `50/min`
  - `Tushare` 对次新股/新股时间窗返回空历史时，`Akshare` 回退不再被快扫层提前掐掉
- 实际结果：
  - `long_base_release` 的低并发 `2026-07-09` 实跑中，不再出现此前那种 `Tushare daily 50/min` 频率超限日志
  - `daily_slow_rise` 虽然当日仍为 `0` 票，但日志已经恢复成真实二段式：
    - 先 `Tushare no data`
    - 再继续尝试 `Akshare`
  - 这说明当前剩余瓶颈已经从“错误地不回退”变成了“真实候选不足 / 免费源自身不稳定”

2026-07-09 addendum (daily review narrowed to one curated document):

- 这轮不是继续加新策略，而是把“默认每日怎么读”收紧。
- 当前默认每日主链路只保留四条真正服务“图形 + 业绩”的信号：
  - `earnings`
  - `hundred_day_high`
  - `daily_slow_rise`
  - `long_base_release`
- `trend_leader` 保留，但降为可选补充观察，不再主导默认每日首页。
- `fast_review_summary.md` 现在的角色也变了：
  - 默认主文档就是这一份
  - 标题改成 `每日精选复盘`
  - 首页优先按 `今日最值得看 / 双确认 / 图形优先 / 业绩优先 / 趋势补充` 组织
  - 原始 `csv/md/txt` 仍保留在同目录，但只作为回查和调试资料
- 以后如果继续调每日复盘，先看：
  - [`docs/个人策略文档/个人每日复盘约定.md`](./个人每日复盘约定.md)
  - 再改脚本和参数，避免重新回到“策略很多、首页却不好看”的状态

2026-07-09 addendum (fast review weaker trend-only demotion + long_base loose default):

- 这轮没有改 `trend_leader`、`hundred_day_high`、`earnings` 三条策略本身的入选阈值。
- 处理的是每日复盘第一页“谁排在前面”的阅读偏差。
- 触发原因很直接：
  - 实跑里像 `大族激光 / 中国巨石` 这类 `trend_only` 样本，虽然仍能命中趋势模型，但当日强度、位置确认和最近价格状态已经不够新鲜。
  - 旧的 `strategy_focus` 排序更看整体分数、资金和板块字段，对“近期已经偏弱的回踩票”惩罚不够。
- 当前快复盘口径补了两层收口：
  - `long_base_release` 在 fast review 默认档切到 `loose`
    - 目的不是放飞样本池，而是让“更像慢推释放”的结构样本有机会进入每日复盘默认视野
    - 独立脚本 `default` 档仍保留，只有 fast-review 默认链路和 repo 默认 profile 改到 `loose`
  - `strategy_focus` 对 `trend_only` 样本新增近期强弱调整
    - 当日涨幅明显偏弱、`stage2` 未确认、`bias_ma5` 偏弱、且不在近新高附近时，会下调排序分
    - 当日强势涨停/大阳、`stage2` 已确认、均线乖离更健康的纯趋势票，会得到额外加分
    - `pullback` 型样本如果当天只有弱修复，不再仅靠旧的 `overall_score + 板块/辨识度` 顶在前面
    - 纯 `trend_only` 行也不再因为同日 bundle 里恰好存在别的 `earnings` 样本，就把趋势行自身残留的 `earnings_strategy_score` 直接当成核心加分证据
- 实际意义：
  - 每日复盘首页会更偏向“最近真的在发力”的趋势样本
  - `long_base_release` 不再因为默认档太紧而长期在 bundle 里空跑
  - 这层改动只收紧阅读层排序，不会改写各子策略落盘的原始候选

2026-07-09 addendum (shared local-strategy universe defaults + long_base_release loose verification):

- 这轮做了两件事：
  - 把本地个人策略默认样本口径收口到共享层，不再让每个脚本各自手写。
  - 给 `long_base_release` 补了一个真正可用于观察池的 `loose` 档，并做了低并发实扫验证。
- 当前共享默认口径：
  - 默认排除 `ST / *ST / PT`
  - 默认排除 `科创板`
  - 默认保留 `创业板`
  - `北交所` 继续由基础 A 股股票池按代码层前置排除
  - 即使关闭 `spot prefilter`，共享 scan shell 也不会把 `ST` 又重新带回样本池
- 已统一接到这套口径的脚本：
  - `monthly_slow_rise`
  - `daily_slow_rise`
  - `long_base_release`
  ...
  - `earnings_surprise`
  - `trend_leader`
  - `warm_local_strategy_cache`
- `long_base_release` 新 `loose` 档思路：
  - 不改 `default`
  - 只额外提供一个更适合观察池/探索池的宽松档
  - 相比默认档，放宽了：
    - 横盘区间波动与区间收益容忍度
    - 释放段最小涨幅、上涨占比、回撤容忍度
    - 20 日成交额门槛
- 2026-07-09 低并发实扫：
  - 命令：
    - `./.venv-linux/bin/python scripts/select_long_base_release_candidates.py --profile loose --snapshot-date 2026-07-09 --limit 2000 --max-workers 1 --exclude-st-prefilter --skip-db-persist`
  - 产物目录：
    - `data/manual_runs/long_base_release_20260709_loose_2000_verify_v3/`
  - 核心结果：
    - `universe_size=1713`
    - `evaluated_count=557`
    - `selected_count=3`
    - `failed_count=554`
    - 失败摘要里没有 `history_fetch_failed`
  - 候选：
    - `航天电器`
    - `探路者`
    - `日科化学`
  - 当前 loose 档下主要剩余淘汰原因已回到形态本身：
    - `ma structure not aligned`
    - `average daily amount too low`

2026-07-08 addendum (fast review low-concurrency safe mode + stable history priority):

- 这轮没有改任何 `trend_leader`、`hundred_day_high`、`daily_slow_rise`、`long_base_release` 的选股阈值。
- 处理的是“每日复盘能不能长期在单机稳跑”的运行链路问题。
- 根因分两层：
  - 快扫专用 `A-share manager` 之前仍偏向 `Akshare` 先跑、`Tushare` 再兜底；在免费源抖动时，容易把全市场扫描拖进大量超时 / 退避 / 子进程堆积。
  - `run_fast_review_bundle.py` 虽然本来就支持很多并发参数，但缺少一个明确、可复用的“安全模式”入口，导致每次都要人工拼参数。
- 当前已改为：
  - `src/services/kline_selector_service.py::build_fast_a_share_manager()`
    - 当 `Tushare` token 可用时，日线历史优先走 `Tushare`
    - `Akshare` 保留为轻量 fallback
    - `Akshare` 在这个快扫 manager 里仍只保留 `tencent/em` 历史路径与较低 retry，避免回到更激进的免费源链路
  - `scripts/run_fast_review_bundle.py`
    - 新增 `--safe-mode`
    - 打开后会把以下并发统一压到 `1`：
      - `external_parallelism`
      - `max_workers`
      - `earnings/hundred_day/monthly/daily/long_base_release/trend/continuous` 各自 worker
- 实际意义：
  - 日常单机跑每日复盘时，不再默认让最不稳定的数据源顶在最前面。
  - 当机器资源紧、或只想求稳时，可以直接用：
    - `python scripts/run_fast_review_bundle.py --strategy-profile-file config/local_strategy_profile.json --snapshot-date 2026-07-08 --safe-mode`
  - 这条模式的目标是“稳定可持续运行”，不是追求最快 wall-clock。

2026-07-09 addendum (local-first cache warming for K-line + earnings):

- 这轮不改任何策略阈值，处理的是“本地缓存怎么长期复用”。
- 当前缓存语义分两层：
  - 日线历史：
    - `DataFetcherManager.get_daily_data(...)` 本来就支持磁盘历史缓存、覆盖区间直接复用、缺口增量补齐。
    - 重点不再是“有没有缓存能力”，而是要主动把常用样本池先热起来。
  - 业绩上下文：
    - 现在除了正常 TTL 内复用，还支持：
      - 当磁盘里的 `earnings_fundamental_context` 虽然过了 TTL
      - 但缓存里对应的 `report_date / report_period` 仍然命中当前报告期
      - 则允许继续复用这份 stale cache
      - 但如果当前报告期里已经出现了更晚的 `report / quick report / forecast` 公告日期，则不会继续复用旧缓存
    - 失败态缓存不走这条长期复用逻辑，仍保持短 TTL，避免把坏状态钉住。
- 新增工具：
  - `scripts/warm_local_strategy_cache.py`
  - 默认目标：
    - 低并发、顺序运行
    - 先构建一个更适合日常复盘的样本池
    - 再预热这批样本的 K 线历史与业绩上下文
    - 当 `snapshot-date` 就是当前有效交易日时，会先按时效刷新当前报告期的业绩事件目录，再决定哪些业绩缓存还能复用
  - 默认过滤：
    - 剔除 `ST`（含 `*ST / PT`）
    - 剔除 `科创板`
    - 剔除 `北交所`
    - 保留 `创业板`
    - `ST` 判断默认优先按 `code` 对齐最新 `stock_basic` 名称，不只依赖 spot 当下那一列名称
  - 默认覆盖：
    - `--top-n <= 0` 时，K 线预热默认覆盖全部过滤后样本
    - `--allowed-prefixes` 默认留空，不再额外按代码前缀缩小样本池
  - 默认产物目录：
    - `data/runtime/cache_prewarm/<snapshot-date>/`
    - 会落盘：
      - `ranked_universe.csv`
      - `kline_cache_warm_results.csv`
      - `earnings_cache_warm_results.csv`
      - `summary.json`
- 常用命令：
  - `python scripts/warm_local_strategy_cache.py --snapshot-date 2026-07-08 --top-n 0 --earnings-top-n 300`
  - 如果只想小范围试跑：
    - `python scripts/warm_local_strategy_cache.py --snapshot-date 2026-07-08 --top-n 300 --earnings-top-n 80 --limit 800`
  - 如果你就是想每次都先强刷业绩事件目录：
    - `python scripts/warm_local_strategy_cache.py --snapshot-date 2026-07-08 --force-refresh-event-catalog`
- 实际意义：
  - 后续 `fast review`、`long_base_release`、`daily_slow_rise` 等依赖日线历史的本地策略，可以优先吃已经预热好的本地盘。
  - 业绩块在“当前还是同一报告期”的窗口里，也不会因为几个小时 TTL 到期就重新全量抓一次。
  - 默认样本口径现在和每日主链路保持一致：显式排除 `ST / 科创板`，并依赖基础股票池继续排除 `北交所`，避免不同策略各自跑出不一致的 universe。

2026-07-09 addendum (history_fetch_failed cleanup in shared scan shell):

- 这轮不改 `monthly_slow_rise / daily_slow_rise / long_base_release / hundred_day_high / earnings_surprise` 的选股阈值，处理的是公共取数噪音。
- 两个直接根因已经收口到共享层：
  - `prepare_scan_universe(...)` 在按 `stock_basic` 合并名称时，历史上会把缺失值误写成字面量 `"nan"`，导致原本应被 `ST/PT` 过滤挡掉的旧壳代码漏进样本池。
  - `DataFetcherManager.get_daily_data(...)` 在 `days` 请求命中“次新股已有部分缓存，但补上市前历史失败”的场景下，会直接抛错，导致本应落到 `insufficient_history` 的标的被记成 `history_fetch_failed`。
- 当前已改为：
  - 共享 universe 准备阶段：
    - 文本缺失值不再覆盖原股票名称；
    - 优先按本地 `tushare_stock_basic_list.csv` / `stock_basic` 活跃代码集收口；
    - 继续排除 `ST/*ST/PT`，并额外顺手挡掉明显退市名。
  - manager 历史缓存阶段：
    - 对自动 `days` 请求，若仅“补上市前 head 历史”失败，但本地已存在从上市后开始的部分历史缓存，则保留这段缓存继续返回；
    - 上层因此可以正常把这类票归到 `insufficient_history`，而不是无差别计入 `history_fetch_failed`。
- 2026-07-09 低并发实扫验证（`monthly_slow_rise`, `profile=loose`, `limit=1000`）：
  - 共享扫描过滤后 `universe: 1000 -> 843`
  - 其中 `removed_inactive_code=112`、`removed_st=44`、`removed_delisted_name=1`
  - 扫描 checkpoint 统计中：
    - `history_fetch_failed = 14`
    - `insufficient_history = 3`
  - 对比修复前同类大样本运行里大量历史壳直接落到 `history_fetch_failed` 的状态，这一轮噪音已经明显收敛
...
2. [`docs/个人策略文档/个人每日复盘约定.md`](./个人每日复盘约定.md)：先确认默认每日到底服务什么目标。
3. [`docs/个人策略文档/个人策略中英对照.md`](./个人策略中英对照.md)：把英文策略名和中文叫法对上。
4. [`docs/local_strategies/README.md`](../local_strategies/README.md)：看我们自己的策略文档目录。
5. [`docs/个人策略文档/个人策略基线.md`](./个人策略基线.md)：看当前默认参数和基线口径。
6. [`docs/个人策略文档/个人策略修改记录.md`](./个人策略修改记录.md)：看详细变更、验证命令、实跑证据。
7. [`docs/CHANGELOG.md`](./CHANGELOG.md)：看用户可见变化摘要。
...
- `daily_slow_rise`
- `long_base_release`
- `trend_leader` -> `trend_leader_unified` 当前只作为可选补充观察，不再默认主导首页
- 每日快复盘主文档 `fast_review_summary.md` 现在直接按 `今日最值得看 / 双确认 / 图形优先 / 业绩优先 / 趋势补充` 组织；原始 `csv/.md` 仍保留在同目录做回查，默认不再要求先看这些明细
- 快复盘阅读层会优先复用已有 `reason_summary / cause_tags`，缺失时才做轻量补算，当前不写回 snapshot
...
| --- | --- | --- | --- | --- | --- |
| 核心 | `trend_leader_unified` | `scripts/select_trend_leader_candidates.py` | `trend_leader_unified` | 否 | [`docs/local_strategies/core/trend_leader_unified.md`](./local_strategies/core/trend_leader_unified.md) |
...
| 扩展 | `daily_slow_rise` | `scripts/select_daily_slow_rise_candidates.py` | `daily_slow_rise` | 是 | [`docs/个人策略文档/个人每日复盘约定.md`](./个人每日复盘约定.md) |
| 扩展 | `long_base_release` | `scripts/select_long_base_release_candidates.py` | `long_base_release` | 是 | [`docs/个人策略文档/个人每日复盘约定.md`](./个人每日复盘约定.md) |
...
| 观察 | `continuous_up_ratio` | `scripts/run_fast_review_bundle.py`（内部收集） | `continuous_up_ratio` | 否 | [`docs/个人策略文档/个人策略基线.md`](./个人策略基线.md) |
| 观察 | `continuous_up_streak` | `scripts/run_fast_review_bundle.py`（内部收集） | `continuous_up_streak` | 否 | [`docs/个人策略文档/个人策略基线.md`](./个人策略基线.md) |
